...
actor Strika {
    ...
    public shared ({ caller }) func createBooking(timeSlot : Time.Time) : async Text {
        ...
        // Verify the time slot exists and is available for booking
        let timeSlotKey = Nat.toText(Int.abs(timeSlot));
        switch (textMap.get(timeSlots, timeSlotKey)) {
            case null {
                Debug.trap("Time slot not available: Slot does not exist");
            };
            case (?slot) {
                if (not slot.isLive) {
                    Debug.trap("Time slot not available: Slot is not published");
                };
                if (not slot.isEnabled) {
                    Debug.trap("Time slot not available: Slot is disabled");
                };
                if (slot.startTime <= Time.now()) {
                    Debug.trap("Time slot not available: Slot is in the past");
                };
            };
        };

        // Check if the time slot is already booked
        for (booking in textMap.vals(bookings)) {
            if (booking.timeSlot == timeSlot and booking.status == "confirmed") {
                Debug.trap("Time slot not available: Already booked");
            };
        };
    ...
    // Time Slot Management for Admin Only
    ...
    var timeSlots = textMap.empty<TimeSlot>();

    public shared ({ caller }) func createTimeSlot(startTime : Time.Time, duration : Nat) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
            Debug.trap("Unauthorized: Only admins can create time slots");
        };
    ...
    public shared ({ caller }) func updateTimeSlot(startTime : Time.Time, duration : Nat) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
            Debug.trap("Unauthorized: Only admins can update time slots");
        };
    ...
    public shared ({ caller }) func toggleTimeSlotAvailability(startTime : Time.Time) : async () {
        ...
        switch (textMap.get(timeSlots, Nat.toText(Int.abs(startTime)))) {
            case null {
                Debug.trap("Time slot not found");
            };
            case (?timeSlot) {
                let updatedSlot = { timeSlot with isEnabled = not timeSlot.isEnabled };
                timeSlots := textMap.put(timeSlots, Nat.toText(Int.abs(startTime)), updatedSlot);
            };
        };
    };

    public shared ({ caller }) func toggleTimeSlotLiveStatus(startTime : Time.Time) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
            Debug.trap("Unauthorized: Only admins can toggle time slot live status");
        };
        ...
        switch (textMap.get(timeSlots, Nat.toText(Int.abs(startTime)))) {
            ...
            case (?timeSlot) {
                let updatedSlot = { timeSlot with isLive = not timeSlot.isLive };
                timeSlots := textMap.put(timeSlots, Nat.toText(Int.abs(startTime)), updatedSlot);
            };
        };
    };

    // New function to get all time slots (admin only)
    public query ({ caller }) func getAllTimeSlots() : async [TimeSlot] {
        if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
            Debug.trap("Unauthorized: Only admins can view all time slots");
        };
        Iter.toArray(textMap.vals(timeSlots));
    };

    public query func getAvailableTimeSlots() : async [TimeSlot] {
        let now = Time.now();
        Iter.toArray(
            Iter.filter(
                textMap.vals(timeSlots),
                func(slot : TimeSlot) : Bool {
                    slot.isLive and slot.isEnabled and slot.startTime > now
                },
            )
        );
    };

    // Staff-Only 15-Minute Extensions
    public shared ({ caller }) func extendBooking15Minutes(bookingId : Text) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
            Debug.trap("Unauthorized: Only admins can extend bookings");
        };
        ...
        switch (textMap.get(bookings, bookingId)) {
            ...
            case (?booking) {
                // Check if the immediately following 15-minute segment is not booked
                let extensionStartTime = booking.timeSlot + 30 * 60 * 1000000000; // 30 minutes after booking start
                let extensionEndTime = extensionStartTime + 15 * 60 * 1000000000; // 15 minutes extension

                // Check for overlapping bookings
                for (existingBooking in textMap.vals(bookings)) {
                    if (existingBooking.status == "confirmed") {
                        let existingEnd = existingBooking.timeSlot + 30 * 60 * 1000000000;
                        // Check if there's any overlap with the extension period
                        if (existingBooking.timeSlot < extensionEndTime and existingEnd > extensionStartTime) {
                            Debug.trap("Cannot extend: Following time segment is already booked");
                        };
                    };
                };
                ...
                timeSlots := textMap.put(timeSlots, Nat.toText(Int.abs(extendedSlot.startTime)), extendedSlot);

                let updatedBooking = {
                    booking with
                    timeSlot = booking.timeSlot + 15 * 60 * 1000000000
                };

                bookings := textMap.put(bookings, bookingId, updatedBooking);
            };
        };
    };
    ...
    var businessContactInfo : ?BusinessContactInfo = null;

    public query func getBusinessContactInfo() : async ?BusinessContactInfo {
        businessContactInfo;
    };

    public shared ({ caller }) func updateBusinessContactInfo(info : BusinessContactInfo) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
            Debug.trap("Unauthorized: Only admins can update business contact info");
        };
        businessContactInfo := ?info;
    };
};
